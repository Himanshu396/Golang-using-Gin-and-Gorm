...
func main() {
	...
	router.POST("/person", func(c *gin.Context) {
		...
		Age := c.Query("Age")
		stmt, err := db.Prepare("insert into person (first_name, last_name,Age) values(?,?,?);")
		if err != nil {
			fmt.Print(err.Error())
		}
		_, err = stmt.Exec(first_name, last_name, Age)
	...
	router.PUT("/person", func(c *gin.Context) {
		...
		last_name := c.Query("last_name")
		Age := c.Query("Age")
		stmt, err := db.Prepare("update person set first_name= ?, last_name= ?,Age=? where id= ?;")
		if err != nil {
			fmt.Print(err.Error())
		}
